...
with tab2:
    ...
    st.markdown("---")
    st.markdown("**Illustrious 18 + Fab 4 全一覧**")
    all_idx = pd.DataFrame(
        [(h, _up_label(d) if isinstance(d, int) else d, f"{thr:+d}", a)
         for (h, d, thr, a) in ILLUSTRIOUS_18 + FAB_4],
        columns=["ハンド", "ディーラー", "発動TC", "アクション"])
...
# ---------------------------------------------------------------------------
with tab5:
    st.subheader("📖 ブラックジャック用語集")
    st.caption("ゲームとシミュレーターで登場するキーワードを解説します。")
...
# ---------------------------------------------------------------------------
with tab6:
    st.subheader("❓ ハウスルール 詳細ガイド")
    st.caption("各設定項目の意味・ハウスエッジへの影響・ENHCとTCの注意点を解説します。詳しい用語の説明は「用語集」タブもご参照ください。")

    def help_term(title, body):
        with st.expander(title):
            st.markdown(body)

    # ── ホールカードルール ──────────────────────────────────────
    st.markdown("#### ホールカードルール（HC / ANHC / ENHC）")

    help_term("HC — US式（Hole Card）", """\
ゲーム開始直後にディーラーが2枚目（ホールカード）をこっそり確認し、BJ（21）なら即ゲーム終了。

- プレイヤーはディーラーがBJでないことを**前提にアクションを選べる**
- Hard 11 vs A = **ダブル**（BJリスクがないためEVがプラス）
- 最もプレイヤーに有利なルール。北米・日本のほとんどのカジノで採用
""")

    help_term("ANHC — オーストラリア式（Australian No Hole Card）", """\
ディーラーは最初から1枚しか配らず、プレイヤーのアクション終了後に2枚目を配る。

- ディーラーBJ時に没収されるのは**元の賭けのみ**（ダブル・スプリット追加分は返還）
- この「ダブル/スプリット保護」により、戦略は**HCとほぼ同一**
- オーストラリア・ニュージーランドのカジノで一般的
""")

    help_term("ENHC — 欧州式（European No Hole Card）", """\
ディーラーBJ時にダブルダウン・スプリットの追加賭け分も**全額没収**。

ディーラーのアップカードが **10 または A** の時、ダブル/スプリットを実行すると、ディーラーBJの場合に賭け金がすべて消える。これによりEVが大きく変動する。

| ハンド | HC | ENHC | 理由 |
|--------|-----|------|------|
| Hard 11 vs A | **D（ダブル）** | H（ヒット） | BJ確率4/13でダブルEV急落（D=−0.35 vs H=−0.10） |
| Hard 14 vs 10 | H（ヒット） | **R（サレンダー）** | BJリスクでev_stand < −0.50 |
| Pair 8,8 vs A | **P（スプリット）** | H（ヒット） | BJリスクでスプリットEV急落（Split=−0.85 vs Hit=−0.64） |
| Pair A,A vs A | **P（スプリット）** | H（ヒット） | Split=−0.35 < Hit=−0.27 |

**TCインデックスの注意**: Dealer 10/A 対面のダブルインデックス（Hard 11 vs A で TC≥+1 → D 等）はENHCでは自動的に無効化されます。
""")

    # ── テーブル基本ルール ──────────────────────────────────────
    st.markdown("#### テーブル基本ルール")

    help_term("デッキ数（1 / 2 / 4 / 6 / 8 デッキ）", """\
デッキ数が少ないほどプレイヤーに有利（ハウスエッジが下がる）。

| デッキ数 | 6D比のエッジ差 | 備考 |
|---------|-------------|------|
| 1デッキ | −0.48% | ピッチゲーム（手配り） |
| 2デッキ | −0.19% | ピッチゲーム |
| 4デッキ | −0.06% | シューゲーム |
| **6デッキ** | **±0%（基準）** | **最も一般的** |
| 8デッキ | +0.02% | 大型カジノ |

デッキ数が多いとカードが平均化されダブルの有利性が薄れる。また**カウンティングの有効性もデッキ数が少ないほど高い**。
""")

    help_term("BJ配当（3:2 vs 6:5）", """\
ブラックジャック（初期2枚で Ace+10点札）が出た時の払い率。

| 配当 | 賭け $100 の払い | ハウスエッジへの影響 |
|------|----------------|-------------------|
| **3:2（標準）** | **$150** | 基準 |
| 6:5 | $120 | **+1.39% 悪化** |

6:5テーブルはカード1枚分を超える不利を生み出す。カウンティングをしても取り返せないレベルのエッジ差があるため、**6:5テーブルには座らないこと**。
""")

    help_term("ディーラー ソフト17（S17 / H17）", """\
ディーラーがソフト17（A+6など合計17のソフトハンド）でどう動くか。
...
H17では A+6（soft 17）からヒットして hand を強化できる（例：10を引いてhard 17、4を引いてsoft 21）。**S17よりディーラーが強い最終手になりやすい**。
""")

    # ── ダブル・スプリット ──────────────────────────────────────
    st.markdown("#### ダブルダウン・スプリット")

    help_term("ダブルダウン条件（any / 9-11 / 10-11）", """\
初期2枚のどの合計値でダブルダウン（賭け倍・1枚のみ引く）できるか。

| 設定 | 対象合計 | ハウスエッジへの影響 |
|------|---------|-------------------|
| **any（推奨）** | 全ての合計 | 基準（最もプレイヤー有利） |
| 9-11 | 9・10・11のみ | +0.09% 悪化 |
| 10-11 | 10・11のみ | +0.18% 悪化 |

`any` ルールでは A+6（soft 17）vs ディーラー3〜6 などでもダブルが有効になる。
""")

    help_term("スプリット後ダブル（DAS）", """\
ペアをスプリット後、さらにダブルダウンできるか。

- **DAS あり**: ハウスエッジを約 **−0.14%** 下げる（有利）
- DAS なし: スプリット後はヒット/スタンドのみ

例: 8,8 をスプリット → 一方の8に3が来て合計11 → DASありならダブル可。
""")

    help_term("エースのスプリット可否", """\
A,A ペアをスプリットできるか。

- **可（推奨）**: A,A は最強のペア。各Aに10系が来れば20〜21に近い手が2つ
- 不可: ほぼ全カジノで可能なため、不可のテーブルは避けるべき

通常、スプリットしたA後に引ける枚数は**1枚のみ**（ほぼ全カジノで標準）。
""")

    help_term("スプリットA後のヒット可否", """\
スプリットしたエースに**追加で**ヒットできるか。

- **不可（標準）**: スプリットA後は1枚のみ追加
- 可（レア）: ハウスエッジが約 **−0.19%** 下がる非常に有利なルール

ラスベガスの一部テーブルや特定条件で提供されることがある。
""")

    help_term("最大スプリット追加回数", """\
同じランクのカードが来た時に何回まで再スプリットできるか。

- **標準: 3回追加**（最大4ハンド）
- 多いほどプレイヤーに有利

例: 8,8 → スプリット → もう1枚8 → 再スプリット（最大4ハンド同時進行）
""")

    # ── サレンダー ──────────────────────────────────────────────
    st.markdown("#### サレンダー")

    help_term("サレンダー（late / none / early）", """\
ゲーム開始後、1アクションも取らずに**賭けの半分を回収して降参**するオプション。

| 種別 | タイミング | ハウスエッジへの影響 |
|------|-----------|-----------------|
| **レイトサレンダー（Late）** | ディーラーBJ確認後 | 約 −0.08% |
| なし（None）| − | サレンダー不可 |
| アーリーサレンダー（Early）| BJ確認前 | 約 −0.39%（非常に有利） |

**サレンダーEV = −0.50**（賭けの半分を失う）。スタンドのEVが −0.50 を下回る絶望的な手でのみ有効。
代表的なサレンダー場面（6D S17 レイトサレンダー）:
- Hard 15 または 16 vs ディーラー10（TC=0）
- Hard 16 vs ディーラー9、A

**重要**: スタンドEV < −0.50 の場合、ヒットよりも**サレンダーが常に優先**される。
""")

    help_term("エース対面でのサレンダー可否", """\
ディーラーのアップカードがAceの時にサレンダーできるか（レイトサレンダー前提）。

HC（US式）では、ディーラーがまずBJを確認する。非BJと確認後にサレンダーできる。

- **不可（デフォルト）**: ディーラーAce時はサレンダー不可
- 可: BJ非確認後にサレンダー可。ハウスエッジ約 **−0.03%** 追加
""")

    # ── ペネトレーション ────────────────────────────────────────
    st.markdown("#### ペネトレーション")

    help_term("ペネトレーション（シミュレーション用）", """\
...
**CSM（連続シャッフルマシン） = ペネトレーション0%** でカウンティング完全無効化。

ペネトレーションが高いほど、TCの精度が増しカウンターのアドバンテージが上がる。逆にカジノは浅いペネトレーション（50%以下）でカウンターを無力化できる。
""")

    # ── True Count とインデックスプレイ ─────────────────────────
    st.markdown("#### True Count（TC）とインデックスプレイ")

    help_term("TC が高い（+）時の戦略変化", """\
**True Count が高い = デッキに10点札（10/J/Q/K）が多く残っている状態**
...
""")
    ...
    help_term("TC が低い（−）時の戦略変化", """\
**True Count が低い = デッキに小さいカード（2〜6）が多く残っている状態**

- ヒットしてもバストしにくい → 積極的にヒット
- **13 vs 2**（TC≤−1）、**12 vs 5**（TC≤−2）などで**Hitに切り替える**
- ダブルダウンの有利性が下がるセルもある（BSでのDマージンが薄い手）
""")

    help_term("ENHC でのTCインデックス注意事項", """\
ENHCでは、ディーラーがBJを事前確認しないため、**dealer 10/A 対面のダブルインデックスは無効化**されます。

**HC でのみ有効なインデックス（ENHCでは除外）**:
| インデックス | 発動TC | HC | ENHC |
|------------|-------|-----|------|
...
dealer 2/7 などBJリスクのないアップカード対面のダブルインデックス（9 vs 2 など）はENHCでも有効です。
""")
